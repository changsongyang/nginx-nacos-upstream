...
typedef struct {
    ...
    u_char  sym;
    u_char  ending;
} http_v2_huff_decode_code_t;


static ngx_inline ngx_int_t http_v2_huff_decode_bits(u_char *state,
    u_char *ending, ngx_uint_t bits, u_char **dst);


static http_v2_huff_decode_code_t  http_v2_huff_decode_codes[256][16] =
{
    /* 0 */
    {
...
ngx_int_t
ngx_nacos_http_v2_huff_decode(u_char *state, u_char *src, size_t len, u_char **dst,
    ngx_uint_t last, ngx_log_t *log)
{
    ...
    while (src != end) {
        ...
        if (http_v2_huff_decode_bits(state, &ending, ch >> 4, dst)
            != NGX_OK)
        {
        ...
        if (http_v2_huff_decode_bits(state, &ending, ch & 0xf, dst)
            != NGX_OK)
        {
            ngx_log_debug2(NGX_LOG_DEBUG_HTTP, log, 0,
                           "http2 huffman decoding error at state %d: "
...
http_v2_huff_decode_bits(u_char *state, u_char *ending, ngx_uint_t bits,
    u_char **dst)
{
    http_v2_huff_decode_code_t  code;

    code = http_v2_huff_decode_codes[*state][bits];
